...
class Wizard:
    ...
    def default_check(self, message: Message) -> bool:
        return all((message.author == self.commander,
    ...
    async def start(self, location: Messageable) -> None:
        ...
        self.embed.add_field(name='Description',
                             value=self.prompts[self.step].description, inline=False)
    ...
    async def get_bool_rr_input(self, prompt):
        ...
        await self.message.add_reaction("\u274E")
        checks = self._combine_checks(
            self.default_reaction_check, prompt.check)

        def bool_check(r, u):
            return checks(r, u) and r.emoji in ("\u2705", "\u274E")
        r, _ = await self.bot.wait_for('reaction_add', check=bool_check, timeout=prompt.timeout)
        await self.message.clear_reactions()
        return r.emoji == "\u2705"
    ...
    async def get_enum_rr_input(self, prompt: Prompt):
        ...
        def enum_check(r, u): return (
            checks(r, u) and
            127462 + opt_len > ord(r.emoji) > 127461
        )

        reaction, _ = await self.bot.wait_for('reaction_add', check=enum_check, timeout=prompt.timeout)
    ...
    async def get_enum_input(self, prompt: Prompt):
        ...
        def enum_check(m): return (
            checks(m) and
            len(m.content) == 1 and
            (
                ord(m.content) in range(65, 65 + opt_len) or
                ord(m.content) in range(97, 97 + opt_len)
            )
        )
        msg = await self.bot.wait_for('message', check=enum_check, timeout=prompt.timeout)
        return list(prompt.type)[ord(msg.content) -
                                 (msg.content.islower() * 32) - 65]
    ...
    async def _get_input(self, prompt):
        ...
        if prompt.type is Reaction:
            result, _ = await self.bot.wait_for('reaction_add', check=self._combine_checks(self.default_reaction_check, prompt.check), timeout=prompt.timeout)
            await self.message.clear_reactions()
            return result
        ...
        while True:
            response = await self.bot.wait_for('message', check=self._combine_checks(self.default_check, prompt.check), timeout=prompt.timeout)
    ...
    async def step_forward(self) -> UserInputable:
        ...
        if prompt.type is str and len(result) > 1024:
            ...
            self.embed.set_field_at(
                self.step, name=prompt.title, value="```Content exceeds embed's limit!```", inline=False)

        if self.step + 1 < len(self.prompts):
            next_prompt = self.prompts[self.step + 1]
            self.step += 1
            self.embed.set_field_at(-1, name='Description',
                                    value=next_prompt.description, inline=False)
    ...
    async def confirm_inputs(self):
        ...
        while True:
            try:
                while True:
                    ...
                    if len(self.prompts) >= field_no >= -1:
                        ...
                        await self.on_invalid_input(commands.CommandError("Invalid input"))
            ...
            self.embed.set_field_at(
                field_no, name=prompt.title, value=to_str(result))
            # self.embed.remove_field(-2)
            await self.update_message()

    async def run(self, location: Messageable) -> Union[List[UserInputable], Mapping[str, UserInputable]]:
        ...
        await self.update_message()
        if self.return_dict:
            return {self.prompts[i].key: self.results[i]
